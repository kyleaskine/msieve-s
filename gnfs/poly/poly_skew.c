...
void find_poly_core(msieve_obj *obj, mpz_t n,
	...
	if (obj->flags & MSIEVE_FLAG_NFS_POLY1) {
		...
		stage1_data.high_coeff_multiplier = 0;  /* default to auto */

		/* check for user-supplied leading coeff multiplier */
		if (obj->nfs_args != NULL) {
			const char *tmp = strstr(obj->nfs_args, "high_coeff_mult=");
			if (tmp != NULL) {
				stage1_data.high_coeff_multiplier = strtoul(tmp + 16, NULL, 10);
